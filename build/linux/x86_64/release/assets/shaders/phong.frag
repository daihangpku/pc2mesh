...
vec3 Shade(vec3 lightIntensity, vec3 lightDir, vec3 normal, vec3 viewDir, vec3 diffuseColor, vec3 specularColor, float shininess) {
    ...
    if (u_UseBlinn) {
        ...
        float specularIntensity = pow(max(dot(normal, H), 0.0), shininess);
        specular = specularIntensity * specularColor;
    } else {
        vec3 R = reflect(-lightDir, normal); 
        float specularIntensity = pow(max(dot(R, viewDir), 0.0), shininess);
        specular = specularIntensity * specularColor;
    }
...
vec3 GetNormal() {
    ...
    vec3 tangent = normalize(dpdx * texCoord_dy.y - dpdy * texCoord_dx.y);
    vec3 bitangent = normalize(dpdy * texCoord_dx.x - dpdx * texCoord_dy.x);

    float heightCenter = texture(u_HeightMap, v_TexCoord).r;
    float height_dx = texture(u_HeightMap, v_TexCoord + vec2(dFdx(v_TexCoord.x), 0.0)).r;
    float height_dy = texture(u_HeightMap, v_TexCoord + vec2(0.0, dFdy(v_TexCoord.y))).r;

    vec3 bumpOffset = vec3((height_dx - heightCenter) * tangent + (height_dy - heightCenter) * bitangent);
    vec3 bumpNormal = bumpOffset;
